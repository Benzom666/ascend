...
const normalizeImages = (images) => {
    ...
    if (typeof images === "object") {
        const url = images.url || images.location || "";
        return url ? [url] : [];
    }
    return [];
};
...
exports.userResponse = (user) => ({
    ...
    images: (() => {
        const verifiedImages = normalizeImages(user.images);
        if (verifiedImages.length > 0) return verifiedImages;
        return normalizeImages(user.un_verified_images);
    })(),
    ...
    un_verified_images: normalizeImages(user.un_verified_images),
    un_verified_profile_details: user.un_verified_profile_details || {},
    ...
    interested_tokens_max: Math.max(user.interested_tokens_max || 0, user.interested_tokens || 0),
    super_interested_tokens_max: Math.max(
        user.super_interested_tokens_max || 0,
        user.super_interested_tokens || 0
    ),
    chat_tokens_max: Math.max(user.chat_tokens_max || 0, user.chat_tokens || 0),
...
exports.imageUploadResponse = function (uploadedObject) {
    const files = [];
