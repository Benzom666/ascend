...
components:
  schemas:
    ...
    WebhookNotification:
      type: object
      description: >
        The JSON body POSTed to your `webhook_url` when a payment status
        changes.
      required:
        - id
        - type
      ...
      properties:
        id:
          ...
          examples:
            - w9c1d2e3-f4a5-6789-bcde-f01234567890
        type:
          type: string
          description: Notification category (always `PAYMENT` for payment events)
          examples:
            - PAYMENT
        event:
          type: string
